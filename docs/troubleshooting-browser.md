...
**Runtime bridge (Cloudflare):** `app/layout.tsx` sets `window.__NEXT_PUBLIC_RUNTIME__` (see `lib/env/injected-next-public.ts`) to an object whose **keys are the same strings as env vars**: `NEXT_PUBLIC_SUPABASE_URL`, and either `NEXT_PUBLIC_SUPABASE_ANON_KEY` and/or `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`. That global name is **not** something you add in GitHub or Cloudflare — it only mirrors Worker `process.env` / `env.client.generated.ts`. Client code reads it in `lib/supabase/resolve-public-env.ts` before falling back to the bundle. After a deploy, bump the service worker cache (`public/sw.js` `CACHE_NAME`) and hard-reload if you still see old behaviour.
