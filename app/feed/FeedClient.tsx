...
interface FeedClientProps {
  ...
  initialConflictDay: number | null;
}

export default function FeedClient({ initialArticles, initialConflictDay }: FeedClientProps) {
  const maxConflictDay = useConflictDay() ?? initialConflictDay;
  ...
  const hasActiveFilters = region !== null || sentiment !== null || conflictDay !== null;

  // Only use the client-side hook when filters are applied
  const {
    articles: filteredArticles,
    loading: filteredLoading,
    error: filteredError,
  } = useArticles(
    hasActiveFilters ? { region, sentiment, conflict_day: conflictDay } : { region, sentiment, conflict_day: conflictDay },
    50
  );

  // Use server data for initial unfiltered render, client data once filters change
  const articles = hasActiveFilters ? filteredArticles : (filteredArticles.length > 0 ? filteredArticles : initialArticles);
  const loading = hasActiveFilters ? filteredLoading : (filteredArticles.length === 0 && filteredLoading && initialArticles.length === 0);
